...
if(isset($_POST['delete'])){
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `admin` WHERE UserID = ?");
...
<section class="grid">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_admins = $conn->prepare("SELECT user.*, admin.* FROM `user` INNER JOIN `admin` ON user.UserID = admin.UserID WHERE user.FullName LIKE ?");
         $select_admins->execute(["%{$search_box}%"]);
      }else{
         $select_admins = $conn->prepare("SELECT user.*, admin.* FROM `user` INNER JOIN `admin` ON user.UserID = admin.UserID");
         $select_admins->execute();
      }
      if($select_admins->rowCount() > 0){
         while($fetch_admins = $select_admins->fetch(PDO::FETCH_ASSOC)){
   ?>
   <?php if( $fetch_admins['UserID'] == $user_id){ ?>
   <div class="box" style="order: -1;">
      <p>name : <span><?= $fetch_admins['FullName']; ?></p>
   ...
   <div class="box">
      ...
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_admins['UserID']; ?>">
